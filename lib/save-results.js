...
module.exports = async (msg) => {
  ...
  if (process.env.GITHUB_ACTION) {
    console.log('Github Action detected, using auth-action');
    const auth = createActionAuth();
    params.authStrategy = createActionAuth;
    params.auth = await auth();
  } else {
    console.log('Using local token');
    params.auth = process.env.GITHUB_TOKEN;
  }
